...
CuEnum_MakeEnum(ServerOperator, Auth, Exit, MsMv, MsLD, MsLU, MsRD, MsRU, KeDn, KeUp);
...
#define MakeExcept__Impl(ex, ...)                                                                    \
    ex(                                                                                              \
        CuStr::Appends(                                                                              \
            CuUtil::String::Combine(                                                                 \
                "[", #ex, "] [", CuUtil_Filename, ":", CuUtil_LineString, "] [", __FUNCTION__, "] ") \
                .data(),                                                                             \
            CuStr::Format(__VA_ARGS__), "\n",                                                        \
            boost::stacktrace::to_string(boost::stacktrace::stacktrace())))
#define MakeExcept(...) MakeExcept__Impl(Exception, __VA_ARGS__)
#define MakeApiExcept(api, ...) MakeExcept__Impl(ApiException, "[{}] {}", api, CuStr::Format(__VA_ARGS__))

// copy from qt
CuEnum_MakeEnumWithType(RcKey, uint32_t,
